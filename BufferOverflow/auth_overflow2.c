...
0000000000400635 <main>:
  ...
  40067b:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)
   ...
   Больше локальных данных в фокции <check_authentication> нет => размер локальных = 16 байт.
   Размер указателя кадра, учитывая 64 битные регистры, = 8 байт.

   Следовательно:

   Для перезаписи адреса возврата потребуется вписать
   16 любых символов + 8 лубых символов + <адрес granted> (1 символ - 1 байт занимаемого места).
